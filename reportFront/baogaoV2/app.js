...
if (document.getElementById('scorePanel')) {
  ...
  cards.forEach(card => card.addEventListener('click', () => {
    sessionStorage.setItem('longevityReportScrollY', String(window.scrollY));
  }));
  const savedScrollY = Number(sessionStorage.getItem('longevityReportScrollY'));
...
const detailRoot = document.querySelector('.detail-v2');
if (detailRoot) {
  ...
  const detailProductRecommendation = document.getElementById('detailProductRecommendation');
  const detailProductIssue = document.getElementById('detailProductIssue');
  const detailProductContext = document.getElementById('detailProductContext');
  ...
  if (data.recommendation && detailProductRecommendation) {
    detailProductRecommendation.hidden = false;
    if (detailProductIssue) detailProductIssue.textContent = `对应问题 · ${data.recommendation.issue}`;
    if (detailProductTitle) detailProductTitle.innerHTML = data.recommendation.title.replace('\n', '<br>');
    if (detailProductContext) detailProductContext.textContent = data.recommendation.context;
    if (detailProductImage) { detailProductImage.src = data.recommendation.image; detailProductImage.alt = data.recommendation.imageAlt; }
  }
