...
import { ReportApiError, fetchSystemDetail, reportParamsFromLocation } from '@/api/reportClient.js'
...
async function load() {
  ...
  state.value = 'loading'
  try {
    const dto = await fetchSystemDetail(reportCode, currentId.value, customerId)
