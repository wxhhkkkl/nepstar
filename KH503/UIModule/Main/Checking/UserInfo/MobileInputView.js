...
export default class MobileInputView extends ActionButtonsMixin(PureComponent) {
    constructor(props){
        super(props)
        this.state = {
            age:'',
    ...
    _nextAction = async ()=>{
        ...
        const {name,age,height,weight,mobile,selected} = this.state
        if(M.isEmpty(name)){
            this._allowNextButton = true
            M.showToast('请输入姓名')
            return
        ...
        try {
            const data = await cloudManager.checkSalesmanMobile(mobile)
        } catch (error) {
