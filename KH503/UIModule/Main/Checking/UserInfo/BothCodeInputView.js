...
const TAG = 'RN_BOTH_CODE_INPUT_VIEW'
const LOG_TAG = '用户信息输入界面'
export default class BothCodeInputView extends ActionButtonsMixin(PureComponent) {
    constructor(props){
        super(props)
        this.state = {
            age:'',
    ...
    _getVerifyCode = async  ()=>{
        if(!this._allowCode){
            // M.showToast('请勿在短时间内重复获取校验码')
            return
        }
        Logger.appendLogInfo(TAG,'点击获取验证码按键')

        
        if(M.isEmpty(this.state.mobile.trim())){
            M.showToast('请输入手机号')
            return
        }
        ...
        try {
            ...
            const info = await cloudManager.getVerifyCode(this.state.mobile)
            M.showToast('校验码获取成功')
            ...
            if(M.isEmpty(error.msg)){
                M.showToast('验证码获取失败')
                return
            }

            M.showToast(error.msg)
        }
    }
    ...
    _renderMobile = ()=>{
        return (
            modeUtil.createBaseUnit({
                ...
                onFocus:()=>{
                    this.restartOverTimer()
                    Logger.appendLogInfo(LOG_TAG,'点击输入手机号')
                    Picker.hide()
                    console.log(TAG,'text input on focus')
                },
                onChangeText:(text)=>{
    ...
    _renderVerifyCode = ()=>{
        return (
            modeUtil.createVerifyCodeUnit({
                text:this.state.verifyCodeText,
                disable:this.state.verifyCodeDisable,
                onFocus:()=>{
                    Logger.appendLogInfo(LOG_TAG,'点击输入校验码')
                ...
                buttonStyle:{
                    color:this.state.verifyCodeColor
                }
            })
        )
    }
    ...
    _renderSalesmanCode = ()=>{
        return (
            modeUtil.createBaseUnit({
                ...
                keyboardType:'numeric',
                onFocus:()=>{
                    Logger.appendLogInfo(LOG_TAG,'点击输入业务员邀请码')
                    this.restartOverTimer()
                    Picker.hide()
                    console.log(TAG,'text input on focus')
                },
    ...
    _nextAction = async ()=>{
        ...
        const {name,age,height,weight,salesmanCode,mobile,selected} = this.state
        if(M.isEmpty(name)){
            M.showToast('请输入姓名')
            this._allowNextButton = true
            return
        ...
        if(M.isEmpty(this.state.verifyCode)){
            this._allowNextButton = true
            M.showToast('请输入验证码')
            return
        }
        ...
        try {
            ...
            const info = await cloudManager.checkVerifyCode(this.state.mobile,this.state.verifyCode)
            ...
            if(error.code === ErrorCode.SALESMAN_CODE_CHECK_ERROR){
    ...
    render = ()=>{
        return (
            <View style={{
                ...
                <BgMainView />
                <Text style={{
                    marginTop:M.designToPixel(40),
                    fontSize:M.designToPixel(45),
                    color:'white',
                }}>{'请输入以下信息'}</Text>
                <View style={{
                    width:M.designToPixel(639),
                    height:M.designToPixel(680),
