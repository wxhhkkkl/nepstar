...
const Strings = {
    title: '欢迎使用康浩云健康检测',
    titleEN: 'Welcome to use Kanghao Cloud Health Tester',
    subTitle: '输入您的预约码',
    subTitleEN: 'Enter your appointment code',
    confirmText: '确认',
    confirmTextEN: 'Confirm',

    alertDesc: '您的预约码有误，请您重新输入',
    alertDescEN: 'Your your appointment code is wrong. Please re-enter it.'
}
...
const TAG = 'RN_APPOINTMENT_CODE_INPUT_VIEW'
const LOG_TAG = '验证码输入模块'
export default class AppointmentCodeInputView extends ActionButtonsMixin(PureComponent) {
    constructor(props) {
        super(props)
    ...
    componentDidMount() {
        ...
        deviceManager.playSound(SoundId.id_appointment_code_input_title)
        this._resetTimer()
    }
    ...
    _onKeyPress(e) {
        ...
        // if (!this._isDeleting) {
        //     // 插入时，每4个需要插入额外空格
        //     JLog('jiji - this._appointmentCode = ', this._appointmentCode)

        //     if ((this._appointmentCode.length > 0) && (this._appointmentCode.length % 4 === 0)) {
    ...
    _onChangeText(text) {
        ...
        if (this._isDeleting) {
            ...
            this._appointmentCode = text.replace(/\s+/g, "")
    ...
    _onCommitText(text) {
        // JLog('jiji - text = ', this._appointmentCode)
        // 收起键盘后，重置计时器
        this._resetTimer()
    }

    async _onConfirmButtonPressed() {
        console.log(TAG,'on confirm button pressed appointment code:',this._appointmentCode)
        if (!this._appointmentCode.length) {
            return
        }
        // 清除计时器
        this._clearTimer()
        try {
            console.log(TAG,'jiji - this._appointmentCode = ', this._appointmentCode)
            const result = await cloudManager.verifyAppointmentCode(this._appointmentCode)
            console.log(TAG,'cloud verify appointment result:',result)
            const { success, msg } = result
            if (!success) {
                // 失败
                this._showAlert(msg)
                // 错误语音
                deviceManager.playSound(SoundId.id_appointment_code_input_error)
                // 重启计时器
                this._resetTimer()
                return
            }
            // 清除计时器
            this._clearTimer()
            // cloudManager.detectionCode = this._appointmentCode;
            // 成功，进入下一页
            setTimeout(() => {
            ...
            console.log(TAG,'cloud verify appointment error:',error)
            Logger.appendLogInfo(LOG_TAG,'预约码异常,错误:'+error);
            // 错误语音
            deviceManager.playSound(SoundId.id_appointment_code_input_error)
            // 重启计时器
            this._resetTimer()
    ...
    _onKeyDownAction(key = '') {
        ...
        if (key === kInputKey.KeyDelete) {
            ...
            // 去掉所有空格
            this._appointmentCode = this._currentText.replace(/\s+/g, "")
            ...
            // 显示的文字
            if (this._appointmentCode.length 
                && (this._appointmentCode.length % 4 === 0) 
            ...
            // 真正的文字
            this._appointmentCode += key
        }

        // JLog('jiji - _appointmentCode = ', this._appointmentCode)
    ...
    _renderInputView() {
        ...
        // 键盘视图
        const keyboardView = this._renderKeyboardView()
...
const styles = StyleSheet.create({
    ...
    inputViewContainerCSS: {
        flexDirection: 'column',
        position: 'absolute',
        top: PublicMethods.designToPixel(400),
        width: '100%',
        alignItems: 'center'
    },
