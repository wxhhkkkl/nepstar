...
const Strings = {
    title: '请用康浩云APP扫码进行检测',
    titleEN: 'Please use Kanghao Cloud APP to scan code and test',
    subTitle: '检测您的健康状况',
...
const LOG_TAG = '二维码界面';
export default class QRScanningView extends ActionButtonsMixin(PureComponent) {
    constructor(props) {
        super(props)
...
//        Alert.alert('title',modeUtil.needJumpInQRView())
//        Alert.alert('qr info:',QRInfo)

        if(modeUtil.needJumpInQRView()){
            console.log(TAG,'qr info:',QRInfo)
            Logger.appendLogInfo(LOG_TAG,'进入二维码界面 模式:qr info:'+JSON.stringify(QRInfo));

            const { navigation } = this.props
            setTimeout(() => {
                modeUtil.runMode({
                    '1':()=>{
                        navigation.replace(kCheckingModuleName.QRScanningMode1Page)
                    },
                    '30':()=>{
                         navigation.replace(kCheckingModuleName.QRScanningMode1Page)
                     },
                    ...
                    '8':()=>{
                        navigation.replace(kCheckingModuleName.QRScanningMode8And9Page)
                    },
                    '9':()=>{
                        navigation.replace(kCheckingModuleName.QRScanningMode8And9Page)
                    },
                    '10':()=>{
                        navigation.replace(kCheckingModuleName.AppointmentCodeInputView)
                    },
                    'default':()=>{
                        // 进入性别选择页
        ...
        deviceManager.playSound(SoundId.id_scan);
        ...
        cloudManager.getUserInfoCallback = ()=>{
            ...
            if(modeUtil.needJumpCodeInput()){
                console.log(TAG,'mode 0 qr info:',QRInfo)
                setTimeout(() => {
                    // 进入验证码输入页
                    const { navigation } = this.props
                    navigation.replace(kCheckingModuleName.CodeInputPage)
                }, 100);
                return
            }
    ...
    componentWillUnmount() {
        ...
        // const emitter = SharedEventEmitter.sharedEmitter().eventEmitter()
        // emitter.removeListener(kRouterChangeEvent, this._configTimeoutTimer)
    ...
    render() {
        if(modeUtil.needJumpInQRView()){
            return (<View style={{width:1920,height:1080, backgroundColor:'black'}}></View>)
        }
            ...
            <View style = {styles.containerCSS}>
                ...
                {/* <Button 
                    ...
                    onPress = {() => {
                        ...
                        // 去下一页
                        const { navigation } = this.props
                        // navigation.push(kCheckingModuleName.FunctionSelectionPage)
                        navigation.push(kCheckingModuleName.TakePhotoFacePage );
                    }}
                /> */}
                {errorView}
            </View>
