...
- [ ] T019 [US1] Create `nepstarAdmin/frontend/src/api/health.ts` with typed indicator functions (`getTree`, `create`, `update`, `remove` via `@/api/client`)
- [ ] T020 [US1] Add the `health` top-level i18n block with indicator keys (labels, page title, `indicator.*` error copy) to `nepstarAdmin/frontend/src/i18n/zh-CN.json`, `en.json`, `es.json`
- [ ] T021 [P] [US1] Write failing frontend component test `nepstarAdmin/frontend/src/views/health/__tests__/IndicatorList.spec.ts` (mount `IndicatorList.vue` with Element Plus, stub `api/health`; verify tree rows render, L1-with-children cannot be deleted, create dialog emits expected payload)
- [ ] T022 [US1] Implement `IndicatorList.vue` in `nepstarAdmin/frontend/src/views/health/IndicatorList.vue` (L1/L2 `el-table`/tree rows, code+name search, inline add/edit `el-dialog` for L1 & L2, enable/disable toggle, delete `el-popconfirm`, error copy via i18n) — depends T019, T020
- [ ] T023 [US1] Register the route `{ path: '/health/indicators', name: 'HealthIndicators', component: () => import('@/views/health/IndicatorList.vue'), meta: { requiresAuth: true } }` in `nepstarAdmin/frontend/src/router/index.ts`
